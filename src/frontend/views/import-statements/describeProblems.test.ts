...
describe("plan rejections", () => {
  ...
  it("names the parsers that were tried", () => {
    const tried = ["hdfc-cc-csv", "stanc-bank-csv"];
...
describe("problem tones", () => {
  ...
  it("never points a prompt at a file inside the package", () => {
      ...
      planRejection([
        {
          ...
          file_name: "notes.html",
          saved_path: staged("notes.html"),
          candidate_parser_paths: [BANK_PARSER],
        },
        {
          status: "ambiguous",
          file_name: "bank.csv",
          saved_path: staged("bank.csv"),
          matching_parser_paths: [BANK_PARSER, "hdfc-cc-xls"],
        },
        ...(
          [
            "no_preset_for_parser",
    ...
    // Every plan problem has a prompt, and all but a few refusals do.
    expect(prompts.length).toBeGreaterThan(Object.keys(PAYLOADS).length);

    for (const prompt of prompts) {
      for (const guide of Object.values(GUIDES)) {
        expect(prompt).not.toContain(guide.file);
      }
    ...
    // Wherever a prompt has the agent write or change a parser, which is
    // where it asks for a fixture, it says where that work goes.
    for (const prompt of prompts) {
