...
export function ImportInstructions() {
  ...
  const chosen =
    ...
    accounts[0] ??
    null;
  const improve = improveTarget(
    review.data,
    chosen?.account.account_id ?? null,
  );
      ...
      <div className="mt-7">
        ...
        >
          {view.tab !== "ai" ? (
            ...
            <RulesPanel key={view.tab} tab={view.tab} />
          ) : presets.isError ? (
            <LoadError
...
): { href: string; uncategorised: number } | null {
  const waiting = (accounts ?? []).filter((one) => one.uncategorised > 0);
  if (waiting.length === 0) return null;
  const account =
    waiting.find((one) => one.account_id === preferred) ??
    (waiting.length === 1 ? waiting[0] : undefined);
  return account
    ? {
        href: reviewHref(account.account_id, IMPROVE_CATEGORIZATION_TAB),
        uncategorised: account.uncategorised,
      }
    : {
        href: REVIEW_ROUTE,
        uncategorised: waiting.reduce((sum, one) => sum + one.uncategorised, 0),
      };
}
