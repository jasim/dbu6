...
# ///
"""Draw the sample statements `dbu6 seed --statements <dir>` describes.

Reads <dir>/statements.json, written by sample-data.ts beside this file, and
...
Usage: uv run render-statements.py <dir>/statements.json
"""
...
# The package directory, from src/server/seed or dist/server/seed alike. The
# HDFC workbook is drawn by that parser's fixture generator, which is in dbu6's
# repository and not in the published package (no fixture ships).
ROOT = Path(__file__).resolve().parents[3]
FIXTURES = ROOT / "custom-built-parsers" / "hdfc-bank-xls" / "fixtures"
if not (FIXTURES / "generate_fixture.py").exists():
    sys.exit(
        "seed --statements draws statement files with dbu6's fixture generators, "
        "which are only in dbu6's repository, not in an installed package."
    )
