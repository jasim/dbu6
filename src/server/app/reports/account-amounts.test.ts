...
describe("account amounts", () => {
  it("signs income and spending positive and lists accounts without entries", () => {
    const amounts = loadAccountAmounts(readOnlyLedger(ledger(), auth), {
  ...
  it("sums each month with entries", () => {
    expect(loadMonthlyAmounts(readOnlyLedger(ledger(), auth), allTime)).toEqual(
      [
    ...
    expect(
      loadMonthlyAmounts(readOnlyLedger(ledger(), auth), {
        fromDate: "2026-02-01",
        toDate: null,
      }),
...
describe("the grids read the amounts", () => {
  ...
  it("spending breakdown totals the same spending by branch", () => {
    const result = expenseBreakdownReport(
      readOnlyLedger(ledger(), auth),
      allTime,
    );
    ...
    expect(
      result.nodes
        .filter((row) => row.columns.parent_key === null)
        .map((row) => [row.columns.name, row.columns.amount]),
    ).toEqual([
      ["Food", 5500],
      ["Shopping", -500],
    ]);
    expect(result.footerRows?.[0]?.columns.amount).toBe(5000);
  });

  it("monthly summary shows each month's amounts", () => {
    const result = monthlySummaryReport(
      readOnlyLedger(ledger(), auth),
      allTime,
    );
