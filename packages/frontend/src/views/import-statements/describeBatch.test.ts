...
function file(fileName: string): AutoImportPlanFile {
  return {
    ...
    file_name: fileName,
    saved_path: null,
    parser_path: "custom-built-parsers/sample-bank-xls/parser.py",
    account: null,
    institution: null,
    preset_name: "Sample Bank",
  };
...
describe("describeBatch after an import", () => {
  it("heads with the new transactions and says how many need a category", () => {
    expect(
      ...
      next: "From 2 statements. 5 need a category before they can go into your books.",
    });
    expect(
      imported([group("sample-aug.xls", 1, tally({ uncategorized: 1 }))]),
    ).toMatchObject({
      text: "1 new transaction imported",
      next: "From 1 statement. 1 needs a category before it can go into your books.",
    });
  });
  ...
  it("says when every new transaction is categorized", () => {
    ...
    ).toMatchObject({ next: "From 1 statement. All are categorized." });
  });
