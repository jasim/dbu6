...
// repository the agent starts in and never asks the user to find it again.
//
...
// repository, or the name the user dropped when the app kept none.
...
The file is in this repository at ${stagedAt(file)}.
...
The upload is in this repository at ${stagedAt(file)}.
...
// them: the staged copies inside this repository, for the agent to read and
// to post back.
...
The uploads are in this repository at ${list(groupPaths(refusal))}.`;
}
...
Please find out what went wrong, starting from the error class behind this
code (packages/api/app/import-error-response.ts translates each one) and the
import pipeline in packages/api/workflows/statement-import/, and tell me what
the fix is before changing the ledger. ${PII_RULE}
