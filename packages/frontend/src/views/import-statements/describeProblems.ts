    ...
    switch (row.status) {
      ...
      case "unrecognized": {
        ...
        problems.push({
          ...
          context:
            "For each bank and statement format, we use a parser to extract the transaction details accurately. We do not have a parser for this specific format. It can be automatically created using the following AI prompt.",
          facts: [
            tried.length === 0
              ? {
                  label: "Parsers tried",
